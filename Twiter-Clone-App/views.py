...
from models import Tweet, User, followers
from forms import TweetForm, LoginForm, RegisterForm
...
def profile(username):
    ...
    return render_template('profile.html', current_user=user, tweets=tweets, current_time=current_time,\
                           followed_by=followed_by, display_follow= display_follow, who_to_watch= who_to_watch, \
...
def timeline(username):
    ...
    if username:
        ...
        total_tweet= Tweet.query.filter_by(user=user).order_by(Tweet.date_created.desc()).count()
    ...
    return render_template('timeline.html', form=form, tweets=tweets, current_time=current_time,\
                            current_user=user, total_tweets=total_tweet, who_to_watch= who_to_watch, \
                            logged_in_user=current_user, followed_by_count= followed_by_count)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_user = User(name=form.name.data,
                        username=form.username.data, image=image_url, password=generate_password_hash(form.password.data), join_date=datetime.now())
